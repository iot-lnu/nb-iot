...
static void processCommands(const char *taskTag, Command *commands, size_t commandCount, QueueHandle_t sendControlQueue)
{
    ...
    while (1)
    {
        ...
        if ((!xQueueReceive(sendControlQueue, &receivedMessage, portMAX_DELAY)) == pdTRUE)
        {
...
static void tx_task(void *arg)
{
    ...
    size_t commandCount;
    CommandType currentType = COMMAND_TYPE_HTTP; // Change to COMMAND_TYPE_MQTT for MQTT commands
    ...
    vQueueDelete(sendControlQueue); // Clean up the queue (though this point is never reached)
}
...
// Timer callback function for if the controller does not receive any data for 15 seconds
void timeout_callback(TimerHandle_t xTimer)
{
    // Log timeout and send reset command
    uint32_t resetAction = COMMAND_ACTION_RESTART;
    ESP_LOGI("Controller", "No data received for 15 seconds, sending reset command.");
